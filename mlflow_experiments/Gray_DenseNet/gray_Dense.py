...
model = load_model('mlflow_experiments\Gray_DenseNet\gray_Dense.h5')

# Load and preprocess a new image for testing
def preprocess_image(image_path):
    img_array = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)  # Load image in grayscale
    if img_array is None:
        raise ValueError(f"Image not found or unable to load: {image_path}")
    img_array = cv2.resize(img_array, (200, 200))  # Resize to match the input size of the model
...
with mlflow.start_run():
    mlflow.log_param("model", "gray_Dense.h5")
    ...
    model_artifact_path = "model/lung_model"
